...
@Component({
  selector: 'app-add-audiolesson',
  templateUrl: './add-audiolesson.component.html',
  styleUrls: ['./add-audiolesson.component.css']
})
export class AddAudiolessonComponent implements OnInit {
  ...
  saveAudiolesson() {
    this.lessonapi.saveAudio(this.audiolesson.value).subscribe(() => {
      this.toastr.success('Audio lesson Added Succesfully!!...', 'StocksAcademy', {
        timeOut: 1000,
        progressBar: true
      });
  ...
  refresh() {
    this.router.navigate(['lesson/audiolesson'])
  }
}
